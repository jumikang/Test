...
class HGFilter(nn.Module):
    def __init__(self, opt):
        ...
        for hg_module in range(self.num_modules):
            ...
            # self.add_module('m' + str(hg_module), HourGlass(1, 4, 256)) #  num of stack is more important
    ...
    def forward(self, x):
        ...
        tmpx = x
        if self.opt.fan_down == 'ave_pool':
            x = F.avg_pool2d(self.conv2(x), 2, stride=2)
        elif self.opt.fan_down in ['conv64', 'conv128']:
            x = self.conv2(x)
            x = self.down_conv2(x)
        else:
            raise NameError('Unknown Fan Filter setting!')
