...
def calc_error_color(opt, netG, netC, cuda, dataset, num_tests):
    ...
    with torch.no_grad():
        ...
        for idx in range(num_tests):
            ...
            # get the error
            errorG = netG.get_error()
            errorG /= opt.num_stack
            # NET C
            # Phase 1: image filter
            netC.filter(image_tensor)
            # Phase 1.5: taking image feature from netG
            netC.attach(netG.get_im_feat())
            # Phase 2: point query
            netC.query(points=color_sample_tensor, calibs=calib_tensor, labels=rgb_tensor)
            # get the error
            errorC = netC.get_error()

            print('{0}/{1} | Error inout: {2:06f} | Error color: {3:06f}'
                  .format(idx, num_tests, errorG.item(), errorC.item()))
            error_inout_arr.append(errorG.item())
            error_color_arr.append(errorC.item())

    return np.average(error_inout_arr), np.average(error_color_arr)